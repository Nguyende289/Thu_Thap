// React và ReactDOM được cung cấp toàn cục từ các thẻ script trong index.html
...
const App = () => {
  ...
  const fetchData = React.useCallback(async (isManual = false) => {
    ...
    try {
      ...
      const tableUrl = `https://docs.google.com/spreadsheets/d/${GOOGLE_SHEET_ID}/gviz/tq?tqx=out:csv${cacheBuster}`; // Lấy sheet đầu tiên
      const headerUrl = `https://docs.google.com/spreadsheets/d/${GOOGLE_SHEET_ID}/gviz/tq?sheet=Sheet2&tqx=out:csv${cacheBuster}`; // Lấy Sheet2
